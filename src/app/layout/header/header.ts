  ...
  template: `
    <mat-toolbar class="w-full text-white shadow-lg">
      <div class="max-w-[1200px] mx-auto w-full flex items-center justify-between py-3">
        ...
        <!-- Logo -->
        <div class="flex flex-col md:flex-row items-center gap-2 cursor-pointer group">
        ...
        <div class="flex-1 px-6 max-w-xl">
          <div class="relative">
              ...
              <button (click)="menu.toggle()" class="text-2xl ml-7 ">
            ☰
            </button>
            
            
            <input #searchInput type="text" placeholder="ابحث عن منتج..."
              class="w-full border rounded-full py-2 pl-2 pr-6"
              (input)="onSearch(searchInput.value)" />
            <mat-icon class="absolute left-2 top-2.5 text-gray-400">search</mat-icon>
          </div>
        </div>
